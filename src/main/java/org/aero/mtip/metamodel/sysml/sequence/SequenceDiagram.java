...
public class SequenceDiagram extends AbstractDiagram {
  ...
  public SequenceDiagram(String name, String importId) {
    ...
    metamodelConstant = SysMLConstants.SYSML_SEQUENCE_DIAGRAM;
    xmlConstant = XmlTagConstants.SEQUENCEDIAGRAM;
    cameoDiagramConstant = SysMLConstants.SYSML_SEQUENCE_DIAGRAM;
  }
  ...
  @CheckForNull
  protected PathElement createMessagePathElement(DiagramPresentationElement presentationDiagram, Message message, Message previousMessage)
    ...
    if (supplierPE == null || clientPE == null || !(supplierPE instanceof ShapeElement) || !(clientPE instanceof ShapeElement)) {
      return null;
    }

    return PresentationElementsManager.getInstance().createSequenceMessage(message, message.getMessageSort(), (ShapeElement) clientPE,
        (ShapeElement) supplierPE, false, 0, previousMessage, 25);
  }

  private void createDurationConstraintView(Element relationship) {
    com.nomagic.uml2.ext.magicdraw.commonbehaviors.mdsimpletime.DurationConstraint dc =
        (com.nomagic.uml2.ext.magicdraw.commonbehaviors.mdsimpletime.DurationConstraint) relationship;
    ...
    if (!(pe1 instanceof PathElement) || !(pe2 instanceof PathElement)) {
      Logger.log(String.format("Duration Constraint not added to diagram. Constrained elements not path elements for %s.",
